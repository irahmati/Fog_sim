...
class Offload:
    ...
    def __init__(self, num_iot, num_fog, num_time, max_delay):
        ...
        self.action_count_0 = 0
        self.action_count_100 = 0
        self.action_count_fog = 0

        self.task_count_iot = 0
        self.task_count_fog = 0
        self.task_count_cloud = 0
        ...
        for iot in range(self.n_iot):
            ...
            self.Queue_cloud_comp.append(list())
            ...
            for cloud in range(self.n_cloud):
                self.Queue_cloud_comp[iot].append(queue.Queue())
        ...
        self.b_cloud_comp = np.zeros([self.n_iot, self.n_cloud])
        ...
        self.cloud_iot_m = np.zeros(self.n_cloud)
        self.cloud_iot_m_observe = np.zeros(self.n_cloud)


        self.task_on_process_cloud = list()
        ...
        for iot in range(self.n_iot):
            ...
            self.task_on_process_cloud.append(list())
            ...
            for cloud in range(self.n_cloud):
                self.task_on_process_cloud[iot].append({'size': np.nan, 'time': np.nan, 'remain': np.nan})
        ...
        self.process_energy_fog = np.zeros([self.n_time, self.n_iot, self.n_fog])
        self.trans_energy = np.zeros([self.n_time, self.n_iot])

        self.wat_process_delay = np.zeros([self.n_time, self.n_iot])    # total delay
        self.wat_process_energy = np.zeros([self.n_time, self.n_iot])
        self.wat_process_energy_fog = np.zeros([self.n_time, self.n_iot, self.n_fog])
        self.wat_idle_energy = np.zeros([self.n_time, self.n_iot, self.n_fog])
        self.wat_trans_energy = np.zeros([self.n_time, self.n_iot])
        ...
        self.cloud_drop = np.zeros([self.n_iot, self.n_cloud])
    ...
    def step(self, action):
        ...
        iot_action_cloud = np.zeros([self.n_iot], np.int32)
        ...
        for iot_index in range(self.n_iot):
            ...
            if iot_action_local[iot_index] == 1:
                ...
                self.task_count_iot = self.task_count_iot + 1
            ...
            if self.task_on_process_local[iot_index]['remain'] > 0:
                ...
                if self.task_on_process_local[iot_index]['remain'] >= (iot_comp_cap / iot_comp_density):
                    self.process_energy[self.task_on_process_local[iot_index]['time'], iot_index] += iot_comp_cap / iot_comp_density
                    self.wat_process_energy[self.task_on_process_local[iot_index]['time'], iot_index] += (iot_comp_cap / iot_comp_density) * 0.297 * pow(10,9) * pow(10,-27) * pow(iot_comp_cap * 10 * pow(10,9), 2)
                 
                else:
    
                    self.process_energy[self.task_on_process_local[iot_index]['time'], iot_index] += self.task_on_process_local[iot_index]['remain']
                    self.wat_process_energy[self.task_on_process_local[iot_index]['time'], iot_index] += self.task_on_process_local[iot_index]['remain'] * 0.297 * pow(10,9) * pow(10,-27) * pow(iot_comp_cap * 10 * pow(10,9), 2)
                ...
                if math.isnan(self.task_on_process_fog[iot_index][fog_index]['remain']) \
                    ...
                    while not self.Queue_fog_comp[iot_index][fog_index].empty():
                        ...
                        if self.time_count - get_task['time'] + 1 <= self.max_delay:
                            ...
                            print("ERRRORRRRRRRR)))))))))))))0")
                            self.process_delay[get_task['time'], iot_index] = self.max_delay
                            self.process_delay_unfinish_ind[get_task['time'], iot_index] = 1



            #    print(self.task_on_process_fog[iot_index][fog_index], "f_________")
                # PROCESS
                self.fog_drop[iot_index, fog_index] = 0
                if self.task_on_process_fog[iot_index][fog_index]['remain'] > 0:
                    if self.task_on_process_fog[iot_index][fog_index]['remain'] >= (self.comp_cap_fog[fog_index] / iot_comp_density / self.fog_iot_m[fog_index]):
                        self.process_energy_fog[self.task_on_process_fog[iot_index][fog_index]['time'], iot_index, fog_index] += \
                            (self.comp_cap_fog[fog_index] / iot_comp_density / self.fog_iot_m[fog_index]) 
                       
                        self.wat_process_energy_fog[self.task_on_process_fog[iot_index][fog_index]['time'], iot_index, fog_index] += \
                            (5 * (self.comp_cap_fog[fog_index] / iot_comp_density / self.fog_iot_m[fog_index]) * 0.297 * pow(10,9)) / (self.comp_cap_fog[fog_index] * 10 * pow(10,9)) 
                    
                        self.wat_idle_energy[self.task_on_process_fog[iot_index][fog_index]['time'], iot_index, fog_index] += \
                            (0.1 * (self.comp_cap_fog[fog_index] / iot_comp_density / self.fog_iot_m[fog_index]) * 0.297 * pow(10,9)) / (self.comp_cap_fog[fog_index] * 10 * pow(10,9) / self.fog_iot_m[fog_index]) 


                    else:
                        self.process_energy_fog[self.task_on_process_fog[iot_index][fog_index]['time'], iot_index, fog_index] += self.task_on_process_fog[iot_index][fog_index]['remain']
                        self.wat_process_energy_fog[self.task_on_process_fog[iot_index][fog_index]['time'], iot_index, fog_index] += \
                            (5 * self.task_on_process_fog[iot_index][fog_index]['remain'] * 0.297 * pow(10,9)) / (self.comp_cap_fog[fog_index] * 10 * pow(10,9))

                        self.wat_idle_energy[self.task_on_process_fog[iot_index][fog_index]['time'], iot_index, fog_index] += \
                            (0.1 * self.task_on_process_fog[iot_index][fog_index]['remain'] * 0.297 * pow(10,9)) / (self.comp_cap_fog[fog_index] * 10 * pow(10,9) / self.fog_iot_m[fog_index]) 
                    ...
                    #print(self.comp_cap_fog[fog_index] / iot_comp_density / self.fog_iot_m[fog_index])

            #        print(self.task_on_process_fog[iot_index][fog_index], "f_________**")

            #        print(self.comp_cap_fog[fog_index])
            #        print(iot_comp_density)
            #        print(self.fog_iot_m[fog_index])
        ...
        # CLOUD QUEUE UPDATE =========================
        #____________function_______________________________
        for iot_index in range(self.n_iot):

            iot_comp_density = self.comp_density[iot_index]

            for cloud_index in range(self.n_cloud):

                # TASK ON PROCESS
                if math.isnan(self.task_on_process_cloud[iot_index][cloud_index]['remain']) \
                        and (not self.Queue_cloud_comp[iot_index][cloud_index].empty()):
                    while not self.Queue_cloud_comp[iot_index][cloud_index].empty():
                        get_task = self.Queue_cloud_comp[iot_index][cloud_index].get()



                        if self.time_count - get_task['time'] + 1 <= self.max_delay:
                            self.task_on_process_cloud[iot_index][cloud_index]['size'] = get_task['size']
                            self.task_on_process_cloud[iot_index][cloud_index]['time'] = get_task['time']
                            self.task_on_process_cloud[iot_index][cloud_index]['remain'] = self.task_on_process_cloud[iot_index][cloud_index]['size']
            ...
            #    print(self.task_on_process_cloud[iot_index][cloud_index], "_________")

                # PROCESS
                self.cloud_drop[iot_index, cloud_index] = 0
                if self.task_on_process_cloud[iot_index][cloud_index]['remain'] > 0:
                    self.task_on_process_cloud[iot_index][cloud_index]['remain'] = \
                        self.task_on_process_cloud[iot_index][cloud_index]['remain'] \
                        - self.comp_cap_cloud[cloud_index] / iot_comp_density / self.cloud_iot_m[cloud_index]


                #    print(self.task_on_process_cloud[iot_index][cloud_index], "_________**")


                #    print(self.comp_cap_cloud[cloud_index])
                #    print(iot_comp_density)
                #    print(self.cloud_iot_m[cloud_index])



                    # if no remain, compute processing delay
                    if self.task_on_process_cloud[iot_index][cloud_index]['remain'] <= 0:
                        self.process_delay[self.task_on_process_cloud[iot_index][cloud_index]['time'],iot_index] \
                            = self.time_count - self.task_on_process_cloud[iot_index][cloud_index]['time'] + 1
                        self.task_on_process_cloud[iot_index][cloud_index]['remain'] = np.nan


                    elif self.time_count - self.task_on_process_cloud[iot_index][cloud_index]['time'] + 1 == self.max_delay:
                        self.process_delay[self.task_on_process_cloud[iot_index][cloud_index]['time'], iot_index] = \
                            self.max_delay
                        self.process_delay_unfinish_ind[self.task_on_process_cloud[iot_index][cloud_index]['time'],
                                                        iot_index] = 1
                        self.cloud_drop[iot_index, cloud_index] = self.task_on_process_cloud[iot_index][cloud_index]['remain']
                        self.task_on_process_cloud[iot_index][cloud_index]['remain'] = np.nan

                        self.drop_cloud_count = self.drop_cloud_count + 1
                ...
                if self.cloud_iot_m[cloud_index] != 0:
                    self.b_cloud_comp[iot_index, cloud_index] \
                        = np.max([self.b_cloud_comp[iot_index, cloud_index]
                                  - self.comp_cap_cloud[cloud_index] / iot_comp_density / self.cloud_iot_m[cloud_index]
                                  - self.cloud_drop[iot_index, cloud_index], 0])
        ...
        for iot_index in range(self.n_iot):
            ...
            if iot_action_local[iot_index] == 0:
                ...
                if iot_action_cloud[iot_index] == 1:
            ...
            if math.isnan(self.task_on_transmit_local[iot_index]['remain']) \
                    and (not self.Queue_iot_tran[iot_index].empty()) and (iot_action_cloud[iot_index] != 1):
            ...
            if self.task_on_transmit_local[iot_index]['remain'] > 0:
                ...
                if self.task_on_transmit_local[iot_index]['remain'] >= iot_tran_cap[1]:
                    self.trans_energy[self.task_on_transmit_local[iot_index]['time'], iot_index] += iot_tran_cap[1]
                    self.wat_trans_energy[self.task_on_transmit_local[iot_index]['time'], iot_index] += (iot_tran_cap[1]  * 0.27) * 10/iot_tran_cap[1] 
                   
                else:
                    self.trans_energy[self.task_on_transmit_local[iot_index]['time'], iot_index] += self.task_on_transmit_local[iot_index]['remain']
                    self.wat_trans_energy[self.task_on_transmit_local[iot_index]['time'], iot_index] += (self.task_on_transmit_local[iot_index]['remain'] * 0.27) * 10 /iot_tran_cap[1]
                ...
                if self.task_on_transmit_local[iot_index]['remain'] <= 0:
                    ...
                    if self.task_on_transmit_local[iot_index]['fog'] == 100:
                        self.Queue_cloud_comp[iot_index][0].put(tmp_dict)

                        self.task_count_cloud = self.task_count_cloud + 1


                        cloud_index = 0
                        self.b_cloud_comp[iot_index, cloud_index] \
                                = self.b_cloud_comp[iot_index, cloud_index] + self.task_on_transmit_local[iot_index]['size']


                    else:
                        self.Queue_fog_comp[iot_index][self.task_on_transmit_local[iot_index]['fog']].put(tmp_dict)

                        self.task_count_fog = self.task_count_fog + 1
        ...
        self.cloud_iot_m_observe = self.cloud_iot_m
        self.cloud_iot_m = np.zeros(self.n_cloud)
        for cloud_index in range(self.n_cloud):
            for iot_index in range(self.n_iot):
                if (not self.Queue_cloud_comp[iot_index][cloud_index].empty()) \
                        or self.task_on_process_cloud[iot_index][cloud_index]['remain'] > 0:
                    self.cloud_iot_m[cloud_index] += 1
